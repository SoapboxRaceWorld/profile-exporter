...
router.post('/', function(req, res, next) {
    ...
    res.status(200)
    axios.default.get(urljoin(server, "GetServerInformation")).then(function(response) {
        try {
            if (response.data.modernAuthSupport == true) {
                axios.default.post(urljoin(server, `User/modernAuth`), {
                    email,
                    password
                ...
                axios.default.get(urljoin(server, `User/authenticateUser?email=${querystring.escape(email).replace('%40', '@')}&password=${querystring.escape(crypto.createHash('sha1').update(password).digest().toString('hex'))}`)).then(function(response) {
                    const loginStatus = response.data
